...
namespace Sonergia\AlfrescoLaravel\Controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Sonergia\AlfrescoLaravel\Facades\Alfresco;
...
/**
 * Class AlfrescoLaravelController
 * @package Sonergia\AlfrescoLaravel\Controllers
 */
class AlfrescoLaravelController extends Controller
{
     ...
     * @return array
     */
    protected function makeBreadcrumb($object, $last = false): array
    {
    ...
    public function add($id, Request $request)
    {
        ...
        if ($docs) {
            ...
            if ($ret) {
                ...
                if ($ok) {
                    $return->with('success', implode("<br/>", $ok));
                }
                if ($err) {
                    $return->with('error', implode("<br/>", $err));
                }
            ...
            if ($folder) {
                return redirect()
                    ...
                    ->with(
                        [
                            'success' => __(
                                "Carpeta creada correctament amb el nom <strong>:name</strong>",
                                ["name" => $folder->name]
                            )
                        ]
                    );
            ...
            if ($object) {
                ...
                return redirect()
                    ...
                    ->with(
                        [
                            'success' => __(
                                ":type renombrat a <strong>:name</strong>",
            ...
            switch ($request->submitaction) {
                    ...
                    if ($request->folderId) {
                        ...
                        foreach ($selected as $fid) {
                            ...
                            try {
                                ...
                                if ($ret) {
                                    $ok[] = __(
                                        "Arxiu <strong>:name</strong> copiat correctament",
                                        ["name" => $ret->name]
                                    );
                                } else {
                                    $err[] = __("Error copiant l'arxiu <strong>:name</strong>", ["name" => $ret->name]);
                                }
                            } catch (AlfrescoObjectAlreadyExistsException $e) {
                                $err[] = __(
                                    "Ja existeix un arxiu amb el mateix nom <strong>:name</strong> a la carpeta destí",
                                    ["name" => $copied->name]
                                );
                            }
                        }
                    ...
                    if ($request->folderId) {
                        ...
                        foreach ($selected as $fid) {
                            ...
                            try {
                                ...
                                if ($ret) {
                                    $ok[] = __(
                                        "Arxiu <strong>:name</strong> mogut correctament",
                                        ["name" => $ret->name]
                                    );
                                } else {
                                    $err[] = __("Error movent l'arxiu <strong>:name</strong>", ["name" => $ret->name]);
                                }
                            } catch (AlfrescoObjectAlreadyExistsException $e) {
                                $err[] = __(
                                    "Ja existeix un arxiu amb el mateix nom <strong>:name</strong> a la carpeta destí",
                ...
                case "delete":
                    foreach ($selected as $fid) {
                        ...
                        if ($ret) {
                            $ok[] = __("Arxiu <strong>:id</strong> esborrat correctament", ["id" => $fid]);
                        } else {
                            $err[] = __("Error esborrant l'arxiu <strong>:id</strong>", ["id" => $fid]);
                        }
                    }
                    break;
                default:
