...
namespace Sonergia\AlfrescoLaravel\Models;

use Illuminate\Http\UploadedFile;
use Illuminate\Support\Src;

use Sonergia\AlfrescoLaravel\Models\Vendor\Zip\TbsZip;

use Sonergia\AlfrescoLaravel\Models\Vendor\Cmis\CmisService;
...
use Sonergia\AlfrescoLaravel\Models\AlfrescoCmisObject;
use Sonergia\AlfrescoLaravel\Models\AlfrescoDocument;
use Sonergia\AlfrescoLaravel\Models\AlfrescoFolder;
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoConnectionException;
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoObjectNotFoundException;
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoObjectAlreadyExistsException;
use Sonergia\AlfrescoLaravel\Models\Helpers\AlfrescoHelper;
...
class AlfrescoCmisProvider
{
    ...
    public function __construct($settings = false)
    {
        ...
        if (!Str::endsWith($this->alfrescourl, "/")) {
            $this->alfrescourl .= "/";
        }
    ...
    { // throws AlfrescoConnectionException {
        try {
            ...
            if ($this->debug) {
                Log::debug("ALFRESCO: Connecting to CMIS API:" . $apiurl);
            }
    ...
    private function checkInBaseFolder($object)
    {
        ...
        throw new AlfrescoObjectNotFoundException(
            __("Object :name doesn't belong to the current site", ["name" => $object->id])
        );
    }
    ...
    { // throws AlfrescoObjectNotFoundException{
        ...
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Folder [:name] not found in Alfresco", ["name" => $this->basepath])
            );
        }
    }
    ...
    protected function fromCmisObject($o)
    {
        ...
        if ($obj->type == self::TYPE_FOLDER || $obj->type == "F:st:sites" || $obj->type == "F:st:site") {
    ...
    public function getObject($objectId)
    {
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Object ID [:name] not found in Alfresco", ["name" => $objectId])
            );
        }
    }
    ...
    public function getObjectByPath($objectPath)
    {
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Object Path [:name] not found in Alfresco", ["name" => $objectPath])
            );
        }
    }
    ...
    {// throws AlfrescoObjectNotFoundException {
        ...
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Folder ID [:name] not found in Alfresco", array("name" => $folderId))
            );
        } catch (CmisObjectNotFoundException $e) {
            throw new AlfrescoObjectNotFoundException(
                __("Folder ID [:name] not found in Alfresco", array("name" => $folderId))
    ...
    {// throws AlfrescoObjectNotFoundException {
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Folder path [:name] not found in Alfresco", array("name" => $folderPath))
            );
        } catch (CmisObjectNotFoundException $e) {
            throw new AlfrescoObjectNotFoundException(
                __("Folder path [:name] not found in Alfresco", array("name" => $folderPath))
    ...
    {// throws AlfrescoObjectNotFoundException {
        try {
            ...
            //_dump($parent);
            if ($parent) {
                return $this->fromCmisObject($parent);
            }
        } catch (CmisObjectNotFoundException $e) {
            throw new AlfrescoObjectNotFoundException(
                __("Folder ID [:name] not found in Alfresco", array("name" => $folderId))
    ...
    private function doCreateFolder($folderName, $parentfolder)
    {
        ...
        try {
            ...
            throw new AlfrescoObjectAlreadyExistsException(
                __(
                    "Folder ':name' already exists in folder ':path' in Alfresco ",
                    array("name" => $folderName, "path" => $parentfolder->path)
                )
            );
        } catch (CmisObjectNotFoundException $e) {
            throw new AlfrescoObjectNotFoundException(
                __("Folder path ':name' not found in Alfresco", array("name" => $parentfolder->path))
            );
        }
    }
    ...
    {//	throws AlfrescoObjectNotFoundException {
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Document ID [:name] not found in Alfresco", array("name" => $documentId))
            );
        } catch (CmisObjectNotFoundException $e) {
            throw new AlfrescoObjectNotFoundException(
                __("Document ID [:name] not found in Alfresco", array("name" => $documentId))
    ...
    {// throws AlfrescoObjectNotFoundException {
        ...
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Document path [:name] not found in Alfresco", array("name" => $documentPath))
            );
        } catch (CmisObjectNotFoundException $e) {
            throw new AlfrescoObjectNotFoundException(
                __("Document path [:name] not found in Alfresco", array("name" => $documentPath))
    ...
    {// throws AlfrescoObjectNotFoundException, AlfrescoObjectAlreadyExistsException {
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Object ID [:name] not found in Alfresco", array("name" => $objectId))
            );
        }
        ...
        if ($target->isFolder()) {
            ...
            if ($obj->isDocument()) {
                ...
                try {
                    $ret = $this->session->createDocument(
                        $target->id,
                        $obj->name,
                        $properties,
                        $contents,
                    ...
                    throw new AlfrescoObjectAlreadyExistsException(
                        __(
                            "Object with name ':name' already exists in folder :path in Alfresco",
                            array("name" => $obj->name, "path" => $target->fullpath)
                        )
                    );
                }
            } else {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Target ID [:name] is not a folder", array("name" => $folderId))
            );
        }
    }
        ...
        if ($target->isFolder()) {
            ...
            try {
                ...
                throw new AlfrescoObjectAlreadyExistsException(
                    __(
                        "Object with name ':name' already exists in folder :path in Alfresco",
                        array("name" => $obj->name, "path" => $target->path)
                    )
                );
            }
        }
    ...
    {// throws AlfrescoObjectNotFoundException, AlfrescoObjectAlreadyExistsException{
        ...
        try {
            ...
            if ($obj->isDocument()) {
                if (!Str::endsWith($newName, "." . $obj->extension)) {
                ...
                try {
                    ...
                    throw new AlfrescoObjectAlreadyExistsException(
                        __(
                            "Object with name ':name' already exists in folder :path in Alfresco",
                            array("name" => $newName, "path" => $target->fullpath)
                        )
                    );
                }
                $children = $this->getChildren($obj->id);
            ...
            throw new AlfrescoObjectAlreadyExistsException(
                __("Object with name ':name' already exists in the same folder in Alfresco", array("name" => $newName))
            );
        }
    }
    ...
    private function doCreateDocument($parentId, $filename, $filecontent, $filetype = false, $index = 0)
    {
        ...
        try {
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Folder ID [:name] not found in Alfresco", array("name" => $parentId))
            );
        }
        ...
        if ($folder->isFolder()) {
            try {
                ...
                if ($this->isRepeatedOverwrite()) {
                    ...
                    throw new AlfrescoObjectAlreadyExistsException(
                        __(
                            "Object with name ':name' already exists in folder :path in Alfresco",
                            array("name" => $filename, "path" => $folder->path)
                        )
                    );
                }
            }
            ...
            throw new AlfrescoObjectNotFoundException(
                __("Folder [:name] doesn't exist in Alfresco", array("name" => $folder->id))
            );
        }
        return false;
    }
