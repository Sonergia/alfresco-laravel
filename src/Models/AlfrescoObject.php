...
use Sonergia\AlfrescoLaravel\Models\AlfrescoCmisProvider;
use Sonergia\AlfrescoLaravel\Models\AlfrescoRestProvider;
...
abstract class AlfrescoObject
{
    ...
    public function setProperties($properties)
    {
        ...
        if (is_object($properties)) {
            $properties = to_array($properties);
        }
    ...
    {
        if ($this->properties && isset($this->properties[$key])) {
    ...
    public abstract function copyTo($parentId
    );// throws AlfrescoObjectNotFoundException, RepositoryObjectAlreadyExistsException;

    public abstract function copyToPath($parentPath
    );// throws AlfrescoObjectNotFoundException, RepositoryObjectAlreadyExistsException;

    public abstract function moveTo($parentId
    );// throws AlfrescoObjectNotFoundException, RepositoryObjectAlreadyExistsException;

    public abstract function moveToPath($parentPath
    );// throws AlfrescoObjectNotFoundException, RepositoryObjectAlreadyExistsException;
    ...
    {
        if ($this->isFile()) {
            return AlfrescoHelper::getIcon($this->mimetype);
        }
    ...
    {
        if ($this->isFile()) {
            return AlfrescoHelper::getColor($this->mimetype);
        }
