...
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoAuthenticationException;
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoIntegrityException;
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoNotAllowedException;
use Sonergia\AlfrescoLaravel\Exceptions\AlfrescoSizeLimitException;
...
class AlfrescoRestProvider
{
    ...
    private function connect()
    {
        $apiurl = $this->apiUrl();
        if ($this->debug) {
            Log::debug("ALFRESCO: Connecting to Rest API:" . $apiurl);
        }
    ...
    protected function call($method, $url, $args = [])
    {
        ...
        if (!isset($args['auth'])) {
            $args['auth'] = [$this->apiuser, $this->apipwd];
        }
        ...
        try {
            ...
            if ($e->hasResponse()) {
                ...
                switch ($status) {
                    case 404:
                        throw new AlfrescoObjectNotFoundException(__("Object not found in Alfresco"), 404);
                        break;
                    case 401:
                        //Authentication exception
                        throw new AlfrescoAuthenticationException(__("User authentication exception"), 401);
                        break;
                    case 403:
                        //Permissions exception
                        throw new AlfrescoNotAllowedException(__("User doesn't have permission"), 403);
                        break;
                    case 409:
                        //New name clashes with an existing node in the current parent folder
                        throw new AlfrescoObjectAlreadyExistsException(__("Name already exists"), 409);
                        break;
                    case 413:
                    case 507:
                        //size limit
                        throw new AlfrescoSizeLimitException(__("Size limit exceeded"), 507);
                        break;
                    case 422:
                        //name containing invalid characters
                        throw new AlfrescoIntegrityException(__("Integrity Exception"), 422);
                        break;
                    default:
                        break;
                }
    ...
    protected function fromRestObject($o)
    {
        ...
        if (isset($o->entry) && $o->entry->isFolder) {
            return AlfrescoFolder::fromRestFolder($o->entry, $this);
        }

        if (isset($o->list) && $o->list->entries) {
    ...
    public function getChildren($folderId, $objectType = false, $page = 1)
    {
        ...
        if ($objectType) {
            ...
            if ($objectType == "file" || $objectType == "document") {
    ...
    { // throws AlfrescoObjectNotFoundException, AlfrescoObjectAlreadyExistsException{
        ...
        $return = $this->call(
            'POST',
            'nodes/' . $parentfolder->id . '/children',
            [
                'query' => ['include' => 'path'],
                "json" => $params
            ]
    ...
    public function copy($objectId, $folderId)
    {
        ...
        throw new AlfrescoObjectNotFoundException(
            __("Object [:id] not found in Alfresco", array("id" => $objectId))
        );
    }
    ...
    protected function doUpload($parentId, $document)
    {
        ...
        if (!$error) {
            try {
                ...
                return __(
                    "Le fichier <strong>:name</strong> existe déjà dans le répertoire",
                    ["name" => $filename]
                );
            } catch (Exception $e) {
                return __(
                    "Error lors de l\'upload du fichier <strong>:name</strong> dans le répertoire",
                    ["name" => $filename]
                );
            }
        } else {
