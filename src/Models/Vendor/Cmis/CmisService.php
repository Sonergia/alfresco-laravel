...
use Sonergia\AlfrescoLaravel\Models\Vendor\Cmis\CmisRepositoryWrapper;
...
// -- Change log token is an anomoly -- but included in URL as parameter
    ...
    static function getEntryTemplate()
    {
        ...
        <atom:entry xmlns:cmis="http://docs.oasis-open.org/ns/cmis/core/200908/"
            ...
            {CONTENT}
            <cmisra:object>
                <cmis:properties>{PROPERTIES}</cmis:properties>
            </cmisra:object>
        </atom:entry>
        <?php
    ...
    static function getPropertyTemplate()
    {
        ...
        <cmis:value>{properties}</cmis:value></cmis:property{propertyType}><?php
    ...
    static function getContentEntry($content, $content_type = "application/octet-stream")
    {
        ...
        if ($content) {
            return CmisRepositoryWrapper:: processTemplate(
                $contentTemplate,
                array(
     ...
     * @since CMIS-1.0
     */
    public function deleteTree($folderId, $options = array())
    { // Nice to have
        $hash_values = $options;
        $myURL = $this->getLink($folderId, self::LINK_DOWN_TREE);
        $myURL = CmisRepositoryWrapper:: getOpUrl($myURL, $hash_values);
     ...
     * @since CMIS-1.0
     */
    public function appendContentStream($objectId, $content, $content_type, $options = array())
    { //Yes
    ...
    public function addObjectToFolder($objectId, $targetFolderId, $options = array())
    { // Probably
        return $this->postObject(
            $targetFolderId,
            $this->getTitle($objectId),
    ...
    public function removeObjectFromFolder($objectId, $targetFolderId, $options = array())
    { //Probably
        $hash_values = $options;
        ...
        $ret = $this->postEntry(
            ...
            null,
            null,
            array("removeFrom" => $targetFolderId)
        );
        $obj = $this->extractObject($ret->body);
        $this->cacheObjectInfo($obj);
        return $obj;
