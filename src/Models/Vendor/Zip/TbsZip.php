...
namespace Sonergia\AlfrescoLaravel\Models\Vendor\Zip;
...
class TbsZip
{
    ...
    {
        $this->Meth8Ok = extension_loaded(
            'zlib'
        ); // check if Zlib extension is available. This is need for compress and uncompress with method 8.
    ...
    function FileAddStream($Name, $Data, $DataType = false, $Compress = true)
    {
        if (!$DataType) {
            $DataType = self::TBSZIP_STRING;
        }
    }

    function FileAdd($Name, $Data, $DataType = false, $Compress = true)
    {
        if (!$DataType) {
            $DataType = self::TBSZIP_STRING;
        }
    ...
    function CentralDirRead_End($cd_info)
    {
        ...
        $x['p_cd'] = $this->_GetDec(
            ...
            16,
            4
        );          // position of start of central directory with respect to the starting disk number
    ...
    function CentralDirRead_File($idx)
    {
        ...
        if ($x !== 'h:02014b50') {
            return $this->RaiseError(
                "Signature of Central Directory Header #" . $idx . " (file information) expected but not found at position " . $this->_TxtPos(
                    ftell($this->ArchHnd) - 46
                ) . "."
            );
        }
        ...
        if ($meth == 8) {
            if ($Uncompress) {
                if ($this->Meth8Ok) {
                    ...
                    $this->RaiseError(
                        'Unable to uncompress file "' . $NameOrIdx . '" because extension Zlib is not installed.'
                    );
                }
            }
        } elseif ($meth == 0) {
            ...
            if ($Uncompress) {
                $this->RaiseError(
                    'Unable to uncompress file "' . $NameOrIdx . '" because it is compressed with method ' . $meth . '.'
                );
            }
        }
    ...
    function _ReadFile($idx, $ReadData)
    {
        ...
        if ($x !== 'h:04034b50') {
            return $this->RaiseError(
                "Signature of Local File Header #" . $idx . " (data section) expected but not found at position " . $this->_TxtPos(
                    ftell($this->ArchHnd) - 30
                ) . "."
            );
        }
        ...
        if (isset($this->CdFileLst[$idx])) {
            ...
            if ($len == 0) {
                $this->RaiseError(
                    "File Data #" . $idx . " cannt be read because no length is specified in the Local File Header and its Central Directory information has not been found."
                );
            }
        }
    ...
    function FileReplace($NameOrIdx, $Data, $DataType = false, $Compress = true)
    {
    ...
    function Flush($Render = false, $File = '', $ContentType = '')
    {
        if (!$Render) {
            $Render = self::TBSZIP_DOWNLOAD;
        }

        if (($File !== '') && ($this->ArchFile === $File) && ($Render == self::TBSZIP_FILE)) {
            $this->RaiseError(
                'Method Flush() cannot overwrite the current opened archive: \'' . $File . '\''
            ); // this makes corrupted zip archives without PHP error.
            return false;
        }
        ...
        foreach ($this->ReplByPos as $ReplPos => $ReplIdx) {
            ...
            $info_old_len = strlen($b1) + $this->CdFileLst[$ReplIdx]['l_data_c'] + strlen(
                    $b2
                ); // $FileInfo['l_data_c'] may have a 0 value in some archives
        ...
        $this->_PutDec(
            ...
            16,
            4
        ); // p_cd  (offset of start of central directory with respect to the starting disk number)
        $this->OutputFromString($b2);
    ...
    function OutputOpen($Render, $File, $ContentType)
    {
        if (($Render & self::TBSZIP_FILE) == self::TBSZIP_FILE) {
            ...
            if ($this->OutputHandle === false) {
                return $this->RaiseError(
                    'Method Flush() cannot overwrite the target file \'' . $File . '\'. This may not be a valid file path or the file may be locked by another process or because of a denied permission.'
                );
            }
    ...
    function _MsDos_Time($Timestamp = false)
    {
        // convert a date-time timstamp into the MS-Dos format
        $d = ($Timestamp === false) ? getdate() : getdate($Timestamp);
        return ($d['hours'] * 2048) + ($d['minutes'] * 32) + intval(
                $d['seconds'] / 2
            ); // seconds are rounded to an even number in order to save 1 bit
    }
    ...
    function _MsDos_Debug($date, $time)
    {
        ...
        return $y . '-' . str_pad($m, 2, '0', STR_PAD_LEFT) . '-' . str_pad($d, 2, '0', STR_PAD_LEFT) . ' ' . str_pad(
                $h,
                2,
                '0',
                STR_PAD_LEFT
            ) . ':' . str_pad($i, 2, '0', STR_PAD_LEFT) . ':' . str_pad($s, 2, '0', STR_PAD_LEFT);
    }
